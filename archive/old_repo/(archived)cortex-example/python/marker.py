...
def main():
    ...
    m.record_title = 'AO_marker' # required param and can not be empty
    m.record_description = '' # optional param

    # marker input for inject marker. Please see add_markers()
    m.marker_value = "1000" # required param and can not be empty
    m.marker_label = "xxxx" #required param and can not be empty

    # input params for export_record. Please see on_warn_cortex_stop_all_sub()
    m.record_export_folder = 'D:\\Upenn\\OneDrive - PennO365\\robotics research\\LilFlo\\FloSystemV2\\trail_control\\cortex-example\\python\\record_data' # your place to export, you should have write permission, example on desktop
    m.record_export_data_types = ['EEG', 'PM', 'BP']
