...
namespace astra {
    ...
    */
    ...
    public:
		...
		{
			astra_fan_enable(set_.get_handle(), enable);
		}

        bool fan_status()
        {
            int32_t value;
            astra_fan_status(set_.get_handle(), &value);
            return value;
        }

        void enable_fan_flash(bool enable)
		{
			astra_fan_flash_enable(set_.get_handle(), enable);
		}

        bool fan_flash_status()
        {
            int32_t value;
            astra_fan_flash_status(set_.get_handle(), &value);
            return value;
        }
